...
class DataPreprocessor:
	...
	Giữ trạng thái (state) của dữ liệu và gọi các hàm từ DataIO và các Transformer class.
	...
	def load_data(
		...
		if auto_convert_numeric:
			self.data = TypeConverter.auto_convert_numeric_columns(
	...
	def auto_detect_columns(self) -> DataPreprocessor:
		...
		types = TypeConverter.auto_detect_columns(self.data)
	...
	def convert_to_datetime(
		...
		"""
		if self.data is None: raise ValueError("Data not loaded.")
		self.data = TypeConverter.convert_to_datetime(self.data, columns, date_format)
		self.auto_detect_columns()
		return self
	...
	def clean_negative_values(self) -> DataPreprocessor:
		...
		"""
		if self.data is None: raise ValueError("Data not loaded.")
		self.data = OutlierHandler.clean_negative_values(self.data)
		return self
	...
	def handle_missing_values(
		...
		processed_data, new_learned = MissingValueHandler.handle(
	...
	def handle_outliers(
		...
		processed_data = OutlierHandler.handle(
	...
	def encode_categorical(
		...
		processed_data, new_encoders = FeatureEncoder.encode(
	...
	def scale_features(
		...
		processed_data, self.scaler, scaled_cols = FeatureScaler.scale(
	...
	def remove_duplicates(self, subset: Optional[list[str]] = None) -> DataPreprocessor:
		...
		"""
		if self.data is None: raise ValueError("Data not loaded.")
		self.data = DataCleaner.remove_duplicates(self.data, subset)
		return self
	...
	def drop_null_targets(self, target_column: str) -> DataPreprocessor:
		...
		"""
		if self.data is None: raise ValueError("Data not loaded.")
		self.data = DataCleaner.drop_null_targets(self.data, target_column)
		return self
	...
	def drop_features(self, features_to_drop: list[str]) -> DataPreprocessor:
		...
		"""
		if self.data is None: raise ValueError("Data not loaded.")
		self.data = DataCleaner.drop_features(self.data, features_to_drop)
		self.auto_detect_columns()
		return self
