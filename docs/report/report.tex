...
\begin{methodlist}
    ...
    Phương thức tiện ích chạy toàn bộ quy trình EDA theo trình tự: \texttt{summary\_statistics()}, \texttt{data\_distribution()}, \texttt{pairplot\_analysis()}, \texttt{correlation\_analysis()}, \texttt{boxplot\_analysis()}.
\end{methodlist}
\end{classbox}
...
Do đó, ở các bước tiếp theo, nhóm lựa chọn cấu hình tiền xử lý gồm: loại bỏ giá trị khuyết (\texttt{drop}), chuẩn hoá \texttt{standard}, và xử lý ngoại lệ bằng \texttt{zscore} để tiến hành trực quan các kết quả\footnote{Ở đây không thực hiện tối ưu siêu tham số vì cho ra kết quả tệ hơn khi chưa sử dụng tối ưu, nguyên nhân có thể do overfitting quan sát hai hình\ref{fig:compare_models} và \ref{fig:compare_models_optimized}}.
...
\begin{table}[h!]
    ...
    \begin{dashlist}
        ...
        \item Chỉnh sửa slide thuyết trình
    \end{dashlist}
    \\ \hline
